...
3. 每个方案默认显示最高版本; 可在右侧详情的版本下拉中选择历史版本, 查看对应说明和成员。
4. 点击导入, 确认 ZIP 中展示的元数据后填写本地方案名称和外置代码目录。

角色筛选按界面语言显示名称, 关键词搜索同时支持中英文。搜索和筛选以各方案的最新版信息为准。
...
若不同打法导出了同名 ZIP, 保存或上传前请在文件名中加上方案名称以区分, 无须修改包内文件。
...
- 作者和方案名称共同标识一个方案。迭代时保持两者不变, 只修改版本、说明或代码。
- 同一队伍的不同打法使用不同方案名称; 更改作者或方案名称会视为新方案。
- 版本必须为三段非负整数, 例如 `1.0.0`、`1.9.0`、`1.10.0`, 每段除 `0` 外不能以零开头。
  不接受 `v` 前缀、预发布后缀或省略段数。
- 工坊按数字比较版本, `1.10.0` 高于 `1.9.0`; 补传旧版本不会顶替最高版本。
- 同一作者、方案名称和版本不能重复提交, 即使成员或源码不同也必须更新版本号。
